...
(deftest not-empty?
  (dt-test-cases/assert-cases-satisfied-by dt-collection/not-empty?
...
(deftest empty?
  (dt-test-cases/assert-cases-satisfied-by dt-collection/empty?
      ...
      (dt-test-cases/true-case (str "any countable with " x " item(s)")
        :samples (let [string (gen/generate (gen-string-of-length x))
                   ...
                   [string
                    ...
                    (into #{} items)
                    (into {} entries)]))
      (dt-test-cases/false-case (str "any countable with less than " x " item(s)")
        :samples (let [string (gen/generate (gen-string-of-length 0 (- x 1)))
                   ...
                   [string
                    ...
                    (into #{} items)
                    (into {} entries)]))
      (dt-test-cases/false-case (str "any countable with more than " x " item(s)")
  ...
  (doseq
   [x (range 1 11)]
    (dt-test-cases/assert-cases-satisfied-by
      ...
      (dt-test-cases/true-case (str "any countable with less than " x " item(s)")
        :samples (let [string (gen/generate (gen-string-of-length 0 (- x 1)))
                       items (gen/generate (gen-seq-of-length 0 (- x 1)))
                       entries (gen/generate (gen-entries-of-length (- x 1)))]
                   [string
  ...
  (doseq
   [x (range 1 11)]
    (dt-test-cases/assert-cases-satisfied-by
      ...
      (dt-test-cases/true-case (str "any countable with more than " x " item(s)")
        :samples (let [string (gen/generate (gen-string-of-length (+ x 1) 25))
                       items (gen/generate (gen-seq-of-length (+ x 1) 25))
                       entries (gen/generate (gen-entries-of-length (+ x 1)))]
                   [string
